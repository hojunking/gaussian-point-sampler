...
from utils import load_pointcept_data, load_3dgs_data, update_3dgs_attributes, remove_duplicates, voxelize_3dgs
from tqdm import tqdm
import os

def merge_pointcept_with_3dgs(pointcept_dir, path_3dgs, output_dir, prune_methods=None, prune_params=None, k_neighbors=5, use_label_consistency=True, ignore_threshold=0.6, filter_ignore_label=False, voxelize=True, voxel_size=0.02):
    """
    ...
    Args:
        ...
        voxelize (bool): 3DGS 데이터에 Voxelization 적용 여부 (기본값: True).
...
def process_single_scene(scene, input_root, output_root, path_3dgs_root, prune_methods, prune_params, k_neighbors=5, use_label_consistency=False, voxelize=True, voxel_size=0.02):
    """
    단일 scene을 처리하는 함수.
    ...
    try:
        ...
        merge_pointcept_with_3dgs(
            ...
            voxelize=voxelize, voxel_size=voxel_size
        )
    except Exception as e:
        print(f"Error processing scene {scene}: {e}")

def process_scenes(input_root, output_root, scene_list, path_3dgs_root, prune_methods=None, prune_params=None, k_neighbors=5, use_label_consistency=True, num_workers=1, voxelize=True, voxel_size=0.02):
    """
    ...
    for scene in tqdm(scene_list, desc="Processing train scenes", unit="scene"):
        process_single_scene(
            scene, input_root, output_root, path_3dgs_root, prune_methods, prune_params, k_neighbors, use_label_consistency,
            voxelize=voxelize, voxel_size=voxel_size
        )
