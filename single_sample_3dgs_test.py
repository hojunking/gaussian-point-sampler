...
from utils import load_pointcept_data, load_3dgs_data, update_3dgs_attributes, remove_duplicates, remove_outliers_3dgs, save_ply
import os
import open3d as o3d
from plyfile import PlyData, PlyElement


def merge_pointcept_with_3dgs(pointcept_dir, path_3dgs, output_dir, prune_methods=None, prune_params=None, exp="None", k_neighbors=5, use_label_consistency=True):
    """
    ...
    normals_pointcept = pointcept_data['normal']  # Pointcept 점의 법선 정보 로드
    ...
    output_ply_path = os.path.join(output_dir, exp, f"{exp}.ply")
    save_ply(
        points_merged, colors_merged, labels_merged, output_ply_path,
        save_separate_labels=True,
        points_pointcept=points_pointcept,  # Pointcept 점 전달
        points_3dgs=points_3dgs            # 3DGS 점 전달
    )
...
def process_single_scene(scene, input_root, output_root, exp, path_3dgs_root, prune_methods, prune_params, k_neighbors=5, use_label_consistency=True):
    ...
    try:
        ...
        merge_pointcept_with_3dgs(
            pointcept_dir, path_3dgs, output_dir, prune_methods, prune_params, exp,
            k_neighbors=k_neighbors, use_label_consistency=use_label_consistency
        )
